...
<p>Of the ten most-installed free scanners on Google Play, every one shows ads, and their reviews describe fake buttons in them. Many open a link the moment they read it. Verdetto does neither. It is made by one person in Alexandria, Virginia; write any time: <a href="mailto:{EMAIL}">{EMAIL}</a>.</p>
...
<ul>
  ...
  <li><strong>Shortened links.</strong> To show you where a shortened link leads, the app asks the shortening service for its destination and follows the answer to where it lands. Each address in the chain, including the destination, sees a request the way it would see any visit; the page itself is not read or shown.</li>
...
<h2>Your data</h2>
<p>Online lookups are on by default and can be turned off in Settings. Product lookups have a switch of their own under it. While on, only the address, domain, product number, or vehicle number goes to the named services (Open Food Facts and its sisters, Open Library, the German and French national libraries, openFDA for medicines and medical devices, MusicBrainz, Wikidata, the NHTSA vehicle database, NHTSA's recall and crash-test databases, and the EPA fuel-economy database); with them off, nothing leaves the phone. There are no ads, no analytics, and no accounts. The <a href="{href('privacy.html')}">privacy policy</a> has the details.</p>
...
<div class="card callout"><p>Verdetto shows every link before it opens, expands shortened ones when online lookups are on, and flags each of the patterns above on your phone. Free, with no ads, paid for by the people who use it. <a href="{href('index.html')}">See what it does</a>.</p></div>
</div>
"""
...
def main():
    ...
    (HERE / "llms.txt").write_text(
        ...
        "- Scanning and every built-in check run on the phone; online lookups (list updates, shortened-link destinations, "
        "domain age, and product, book, medicine, music, and vehicle details from Open Food Facts, Open Library, the German and "
        "French national libraries, openFDA, MusicBrainz, Wikidata, the NHTSA vehicle, recall, and crash-test databases, and the EPA fuel-economy database) are on by default and can be turned off.\n"
