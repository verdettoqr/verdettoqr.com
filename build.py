...
APP = {"@type": "SoftwareApplication", "name": "Verdetto: QR & Barcode Scanner", "operatingSystem": "Android",
       ...
       "description": "See the link before it opens. Free, no ads, no tracking. Reads the codes other apps give up on."}
...
def stack_tables(doc):
    """Every table gets class "stack" and each body cell a data-label carrying its column header, so the phone
    layout can stack a row into a labeled block (operator, 2026-09-06: the intents table from a phone)."""
    def one(m):
        table = m.group(0)
        labels = [re.sub(r"<[^>]+>", "", x).strip().replace('"', "&quot;")
                  for x in re.findall(r"<th\b[^>]*>(.*?)</th>", table, flags=re.S)]
        if not labels:
            return table
        def row(rm):
            r = rm.group(0)
            ...
            def cell(cm):
                k = next(count)
                return f'<td data-label="{labels[k] if k < len(labels) else ""}"' + cm.group(1)
            return re.sub(r"<td\b([^>]*)", cell, r)
        table = re.sub(r"<tr\b.*?</tr>", row, table, flags=re.S)
        def open_tag(tm):
            attrs = tm.group(1)
            return "<table" + (attrs.replace('class="', 'class="stack ', 1) if 'class="' in attrs else ' class="stack"' + attrs) + ">"
        return re.sub(r"<table\b([^>]*)>", open_tag, table, count=1)
    return re.sub(r"<table\b.*?</table>", one, doc, flags=re.S)
...
<p><strong>Builds from F-Droid or GitHub.</strong> Those builds have no Google Play billing, so there is no Play order. The Support screen opens our website, where a contribution goes to GitHub Sponsors, to Liberapay or to Stripe; the app itself sends nothing and keeps only the badge switch you set on your phone. GitHub handles a sponsorship under <a href="https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement">its own privacy statement</a> and shows us your GitHub username unless you make the sponsorship private, the name and email you have made public on GitHub if any, the amount, the date, and, in a monthly record for tax, the country or region the payment came from and whether it came from a business. Liberapay handles a contribution under <a href="https://liberapay.com/about/privacy">its own privacy policy</a> and shows us the amount and nothing about you, unless you pay through PayPal, which shows us your name and email address as any PayPal payment does. Stripe handles a card payment under <a href="https://stripe.com/privacy">its own privacy policy</a> and shows us the name and email address you type on the payment page, the amount, the date, the card's country and the last four digits of the card; we never see the full card number. We use these records only for refunds, tax and bookkeeping, keep them as long as tax law requires, and never load them into the app.</p>
<p><strong>Google Play's counts.</strong> Google Play shows us, in its developer console, counts of installs, uninstalls, ratings, and crashes, by country and by the site or store page that led to the store page. Google records that when the store page opens, under its own privacy policy; the app does not request it from Google and never sends it anywhere. The counts are grouped and rounded by Google, and we cannot see anyone in them. GitHub shows us how many times a release file was downloaded, as a number only; F-Droid shows us nothing.</p>
...
SUPPORT_WORK_T = {
 "en": {
  ...
  "browser_wait": "Three routes are being set up, listed below. Until one opens, the app is the way to give.",
  ...
  "a_computer_live": "Yes: GitHub Sponsors or Liberapay, once or monthly; a card link, once.",
  ...
  "nothing_p": "A web contribution pays for the work and unlocks nothing: no tier, no extra, none of the items the Play purchase includes.",
  ...
  "routes_p": "On Google Play, support happens inside the app. Every other build sends you here.",
  "r_sponsors": "Once or monthly, from $1, in whole dollars: GitHub's tiers of $1, $3, $5, $10 and $20, or an amount you type.",
 ...
 "de": {
  ...
  "browser_wait": "Drei Wege werden gerade eingerichtet, unten aufgeführt. Bis einer öffnet, ist die App der Weg zu geben.",
  ...
  "a_computer_live": "Ja: GitHub Sponsors oder Liberapay, einmal oder monatlich; ein Kartenlink, einmal.",
  ...
  "nothing_p": "Ein Web-Beitrag bezahlt die Arbeit und schaltet nichts frei: keine Stufe, kein Extra, nichts von dem, was der Play-Kauf einschließt.",
  ...
  "routes_p": "Bei Google Play läuft die Unterstützung in der App. Jeder andere Build schickt dich hierher.",
  "r_sponsors": "Einmal oder monatlich, ab 1 $, in ganzen Dollar: GitHubs Stufen 1 $, 3 $, 5 $, 10 $ und 20 $ oder ein Betrag, den du eingibst.",
 ...
 "es": {
  ...
  "browser_wait": "Se están configurando tres vías, listadas abajo. Hasta que abra una, la aplicación es la forma de dar.",
  ...
  "a_computer_live": "Sí: GitHub Sponsors o Liberapay, una vez o cada mes; un enlace de tarjeta, una vez.",
  ...
  "nothing_p": "Una contribución web paga el trabajo y no desbloquea nada: ni nivel, ni extra, ni nada de lo que incluye la compra de Play.",
  ...
  "routes_p": "En Google Play, el apoyo se hace dentro de la aplicación. Cualquier otra versión te manda aquí.",
  "r_sponsors": "Una vez o cada mes, desde 1 $, en dólares enteros: los tramos de GitHub de 1 $, 3 $, 5 $, 10 $ y 20 $, o un importe que escribes.",
 ...
 "fr": {
  ...
  "browser_wait": "Trois voies sont en cours d'ouverture, listées ci-dessous. Jusque-là, l'application est le moyen de donner.",
  ...
  "a_computer_live": "Oui : GitHub Sponsors ou Liberapay, une fois ou chaque mois ; un lien de paiement par carte, une fois.",
  ...
  "nothing_p": "Une contribution web paie le travail et ne débloque rien : ni palier, ni extra, rien de ce que l'achat Play inclut.",
  ...
  "routes_p": "Sur Google Play, le soutien se fait dans l'application. Toute autre version t'envoie ici.",
  "r_sponsors": "Une fois ou chaque mois, dès 1 $, en dollars entiers : les paliers GitHub de 1 $, 3 $, 5 $, 10 $ et 20 $, ou un montant que tu saisis.",
 ...
 "pt-BR": {
  ...
  "browser_wait": "Três caminhos estão sendo configurados, listados abaixo. Até um abrir, o app é o jeito de contribuir.",
  ...
  "a_computer_live": "Sim: GitHub Sponsors ou Liberapay, uma vez ou todo mês; um link de cartão, uma vez.",
  ...
  "nothing_p": "Uma contribuição pela web paga o trabalho e não desbloqueia nada: nenhum nível, nenhum extra, nada do que a compra no Play inclui.",
  ...
  "routes_p": "No Google Play, o apoio acontece dentro do app. Qualquer outra versão manda você para cá.",
  "r_sponsors": "Uma vez ou todo mês, a partir de US$ 1, em dólares inteiros: os níveis do GitHub de US$ 1, 3, 5, 10 e 20, ou um valor que você digita.",
 ...
 "id": {
  ...
  "browser_wait": "Tiga jalur sedang disiapkan, tercantum di bawah. Sampai ada yang dibuka, aplikasi adalah cara untuk memberi.",
  ...
  "a_computer_live": "Ya: GitHub Sponsors atau Liberapay, sekali atau bulanan; tautan kartu, sekali.",
  ...
  "nothing_p": "Kontribusi lewat web membayar pekerjaan ini dan tidak membuka apa pun: tanpa tingkatan, tanpa tambahan, dan tanpa apa pun yang termasuk dalam pembelian Play.",
  ...
  "routes_p": "Di Google Play, dukungan diberikan di dalam aplikasi. Build lain mengarahkanmu ke sini.",
  "r_sponsors": "Sekali atau bulanan, mulai $1, dalam dolar bulat: tingkatan GitHub $1, $3, $5, $10, dan $20, atau jumlah yang kamu ketik.",
 ...
 "ru": {
  ...
  "browser_wait": "Три пути ещё настраиваются, они перечислены ниже. Пока один из них не открылся, приложение остаётся способом поддержать.",
  ...
  "a_computer_live": "Да: GitHub Sponsors или Liberapay, разово или ежемесячно; ссылка для оплаты картой, разово.",
  ...
  "nothing_p": "Взнос через сайт оплачивает работу и ничего не открывает: ни уровня, ни дополнений, ничего из того, что входит в покупку в Play.",
  ...
  "routes_p": "В Google Play поддержка происходит внутри приложения. Любая другая сборка направляет вас сюда.",
  "r_sponsors": "Разово или ежемесячно, от 1 $, целыми долларами: ступени GitHub 1 $, 3 $, 5 $, 10 $ и 20 $ или сумма, которую вы вводите.",
 ...
 "hi": {
  ...
  "browser_wait": "तीन रास्ते तैयार किए जा रहे हैं, नीचे सूचीबद्ध हैं। जब तक कोई खुले, ऐप ही देने का तरीका है।",
  ...
  "a_computer_live": "हाँ: GitHub Sponsors या Liberapay, एक बार या हर महीने; कार्ड लिंक, एक बार।",
  ...
  "nothing_p": "वेब योगदान काम का भुगतान करता है और कुछ भी अनलॉक नहीं करता: न कोई स्तर, न कोई अतिरिक्त, न Play खरीद में शामिल कोई चीज़।",
  ...
  "routes_p": "Google Play पर सहयोग ऐप के भीतर होता है। हर दूसरा बिल्ड आपको यहाँ भेजता है।",
  "r_sponsors": "एक बार या हर महीने, $1 से, पूरे डॉलर में: GitHub के स्तर $1, $3, $5, $10 और $20, या आपकी लिखी कोई रकम।",
 ...
 "ja": {
  ...
  "browser_h": "ブラウザーから",
  "browser_wait": "三つの経路を準備中です。下に一覧があります。開くまでは、アプリが支援の方法です。",
  ...
  "a_computer_live": "はい。GitHub Sponsors か Liberapay で一度だけでも毎月でも、カード決済のリンクで一度だけ。",
  "a_computer_wait": "まだです。GitHub Sponsors、Liberapay、カード決済のリンクは準備中で、それぞれ開いたらこのページでお知らせします。",
  "soon": "近日公開",
  "routes_h": "F-Droid、GitHub、またはパソコンから",
  "nothing_p": "ウェブでの支援は開発の費用にあてられ、何も解放しません。段階も特典もなく、Play での購入に含まれるものも対象外です。",
  ...
  "routes_p": "Google Play では、支援はアプリの中で行います。それ以外のビルドはこのページに案内します。",
  "r_sponsors": "一度だけでも毎月でも、1 ドルから、ドル単位で。GitHub の段階は 1、3、5、10、20 ドル、または入力した額。",
 ...
 "zh-Hans": {
  ...
  "browser_h": "通过浏览器",
  "browser_wait": "三条途径正在设置中，见下方列表。在其中一条开放之前，应用是支持的途径。",
  ...
  "a_computer_live": "可以：GitHub Sponsors 或 Liberapay，一次或每月；银行卡链接，一次。",
  "a_computer_wait": "还不行。GitHub Sponsors、Liberapay 和银行卡链接正在设置中；各自开放时本页会说明。",
  "soon": "即将推出",
  "routes_h": "通过 F-Droid、GitHub 或电脑",
  "nothing_p": "网页支持用于支付开发工作，不解锁任何东西：没有档位、没有额外内容，也不包括 Play 购买所含的任何内容。",
  ...
  "routes_p": "在 Google Play 上，支持在应用内完成。其他任何版本都会把你带到这里。",
  "r_sponsors": "一次或每月，1 美元起，以整美元计：GitHub 的档位为 1、3、5、10 和 20 美元，或自行输入的金额。",
 ...
 "ar": {
  ...
  "browser_wait": "يجري إعداد ثلاث طرق، مذكورة أدناه. وحتى تُفتح إحداها، يبقى التطبيق هو طريقة العطاء.",
  ...
  "a_computer_live": "نعم: GitHub Sponsors أو Liberapay، مرة واحدة أو شهريًا؛ ورابط الدفع بالبطاقة، مرة واحدة.",
  ...
  "nothing_p": "المساهمة عبر الويب تدفع ثمن العمل ولا تفتح شيئًا: لا درجة ولا إضافة ولا شيئًا مما يشمله شراء Play.",
  ...
  "routes_p": "على Google Play، يتم الدعم داخل التطبيق. وكل إصدار آخر يرسلك إلى هنا.",
  "r_sponsors": "مرة واحدة أو شهريًا، من دولار واحد، بدولارات كاملة: درجات GitHub 1 و3 و5 و10 و20 دولارًا، أو مبلغ تكتبه.",
...
def support_work_body(t, code):
    ...
    def row(flag, url, name, what, anchor=None):
        # the app opens #fdroid or #github per install source (the contract): F-Droid lands on Liberapay, GitHub on Sponsors
        tag = "" if flag and url else f'<span class="tag">{t["soon"]}</span>'
        li = f'<li id="{anchor}">' if anchor else "<li>"
        return f'{li}<span class="name">{route(flag, url, name)}</span><span class="what">{what}</span>{tag}</li>'
    rows = (row(SPONSORS_LIVE, "https://github.com/sponsors/verdettoqr", "GitHub Sponsors", t["r_sponsors"], "github") + "\n"
            + row(LIBERAPAY_LIVE, LIBERAPAY_URL, "Liberapay", t["r_liberapay"], "fdroid") + "\n"
            + row(CARD_LIVE, CARD_URL, t["r_card_h"], t["r_card"]))
    all_live = SPONSORS_LIVE and LIBERAPAY_LIVE and CARD_LIVE
    routes = (f'<h2 id="routes">{t["routes_h"]}</h2>\n<p>{t["routes_p"]}</p>\n<ul class="routes">\n{rows}\n</ul>\n'
              + ("" if all_live else f'<p>{t["until_p"]}</p>\n')
...
HOME_T = {
 "en": dict(title="Verdetto: QR & Barcode Scanner for Android", desc="See the link before it opens. Free, no ads, no tracking. Reads the codes other apps give up on.",
  ...
  why_p="Of the ten most-installed free scanners on Google Play, checked on September 4, 2026, all ten carry ads, and reviews of seven describe a fake button in the ad. Many open a link the moment they read it. Verdetto does neither. {press}. It is made by a solo developer in Virginia; write any time: {email}.",
 ...
 "es": dict(title="Verdetto: escáner de códigos QR y de barras para Android", desc="Mira el enlace antes de que se abra. Gratis, sin anuncios, sin rastreo. Lee los códigos que otras apps abandonan.",
  ...
  why_p="De los diez escáneres gratuitos más instalados en Google Play, comprobados el 4 de septiembre de 2026, los diez llevan anuncios, y las reseñas de siete describen un botón falso en el anuncio. Muchos abren el enlace en cuanto lo leen. Verdetto no hace ninguna de las dos cosas. {press}. Lo hace un desarrollador independiente en Virginia; escribe cuando quieras: {email}.",
 ...
 "fr": dict(title="Verdetto: scanner de codes QR et codes-barres pour Android", desc="Vois le lien avant qu'il ne s'ouvre. Gratuit, sans publicité, sans pistage. Lit les codes que les autres applications abandonnent.",
 ...
 "de": dict(title="Verdetto: QR- und Barcode-Scanner für Android", desc="Sieh den Link, bevor er sich öffnet. Kostenlos, ohne Werbung, ohne Tracking. Liest die Codes, bei denen andere Apps aufgeben.",
 ...
 "pt-BR": dict(title="Verdetto: leitor de QR e código de barras para Android", desc="Veja o link antes de abrir. Grátis, sem anúncios, sem rastreamento. Lê os códigos que outros apps desistem de ler.",
 ...
 "ru": dict(title="Verdetto: сканер QR-кодов и штрихкодов для Android", desc="Увидьте ссылку до того, как она откроется. Бесплатно, без рекламы, без слежки. Читает коды, на которых другие приложения сдаются.",
 ...
 "id": dict(title="Verdetto: pemindai QR dan barcode untuk Android", desc="Lihat tautannya sebelum terbuka. Gratis, tanpa iklan, tanpa pelacakan. Membaca kode yang menyerah dibaca aplikasi lain.",
  ...
  why_p="Dari sepuluh pemindai gratis terbanyak dipasang di Google Play, diperiksa pada 4 September 2026, kesepuluhnya memuat iklan, dan ulasan tujuh di antaranya menggambarkan tombol palsu di iklan. Banyak yang membuka tautan begitu membacanya. Verdetto tidak melakukan keduanya. {press}. Dibuat oleh seorang pengembang independen di Virginia; tulislah kapan saja: {email}.",
 ...
 "ja": dict(title="Verdetto: Android 向け QR・バーコードスキャナー", desc="開く前にリンクを見る。無料、広告なし、追跡なし。他のアプリがあきらめるコードを読みます。",
  h1="開く前にリンクを見る。",
  ...
  why_p="Google Play でインストール数の多い無料スキャナー 10 本を 2026 年 9 月 4 日に確認したところ、10 本すべてに広告があり、7 本のレビューには広告内の偽ボタンが記されていました。多くは読み取った瞬間にリンクを開きます。Verdetto はどちらもしません。 {press}。バージニア州の個人開発者が作っています。いつでもご連絡ください: {email}。",
  never="決して言わないこと",
 ...
 "zh-Hans": dict(title="Verdetto: Android 的二维码和条形码扫描器", desc="在链接打开前先看清它。免费、无广告、无跟踪。读取其他应用放弃的码。",
  h1="在链接打开前先看清它。",
  ...
  why="为什么还要一个二维码扫描器",
  why_p="在 Google Play 安装量最高的十款免费扫描器中（2026 年 9 月 4 日核查），十款全部带广告，其中七款的评价描述了广告里的假按钮。许多会在读取到链接的瞬间就打开它。Verdetto 两者都不做。 {press}。它由弗吉尼亚州的一位独立开发者制作；随时来信：{email}。",
  never="它永远不会告诉你的事",
 ...
 "hi": dict(title="Verdetto: Android के लिए QR और बारकोड स्कैनर", desc="लिंक खुलने से पहले देखें। मुफ़्त, बिना विज्ञापन, बिना ट्रैकिंग। वे कोड पढ़ता है जिन्हें दूसरे ऐप छोड़ देते हैं।",
 ...
 "ar": dict(title="Verdetto: قارئ رموز QR والباركود لنظام Android", desc="شاهد الرابط قبل أن يُفتح. مجاني، بلا إعلانات، بلا تتبّع. يقرأ الرموز التي تستسلم أمامها التطبيقات الأخرى.",
  ...
  why_p="من بين أكثر عشرة قارئات مجانية تثبيتًا على Google Play، والتي فُحصت في 4 سبتمبر 2026، تحمل العشرة كلها إعلانات، وتصف مراجعات سبعة منها زرًا زائفًا داخل الإعلان. وكثير منها يفتح الرابط لحظة قراءته. Verdetto لا يفعل أيًّا من الأمرين. {press}. يصنعه مطوّر مستقل في فرجينيا؛ اكتب في أي وقت: {email}.",
...
def home_body(t, code):
    ...
    features_link = f'<a href="{href(localized("features.html", code))}#reads">{t["reads_link"]}</a>'
    cards = "".join(f'  <div class="card">{ic(i)}<div><h3>{title}</h3><p>{text.replace("{features}", features_link)}</p></div></div>\n'
    ...
    why_p = (t["why_p"].replace("{email}", f'<a href="mailto:{EMAIL}">{EMAIL}</a>')
             .replace("{press}", f'<a href="{href(localized("press.html", code))}#comparison-basis">{t["press_link"]}</a>'))
...
PAGES = {
    "index.html": ("Verdetto: QR & Barcode Scanner for Android", "See the link before it opens. Free, no ads, no tracking. Reads the codes other apps give up on.", HOME, APP),
...
def main():
    ...
    # security.txt (RFC 9116): how to report a security problem, at the well-known address; Jekyll serves the dotfolder
    # through _config.yml's include. Expires stays under a year out; move it forward with each policy date.
    (HERE / ".well-known").mkdir(exist_ok=True)
    (HERE / ".well-known" / "security.txt").write_text(
        "Contact: mailto:" + EMAIL + "\n"
        "Expires: 2027-08-31T00:00:00.000Z\n"
        "Policy: " + SITE + "/privacy\n"
        "Preferred-Languages: en, de, es, fr, pt-BR, id, ru, hi, ja, zh-Hans, ar\n"
        "Canonical: " + SITE + "/.well-known/security.txt\n", encoding="utf-8", newline="\n")
    print("wrote robots.txt, sitemap.xml, llms.txt, CNAME, .well-known/security.txt")
